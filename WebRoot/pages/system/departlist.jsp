...
<body>
	<nav class="breadcrumb">
		...
		权限管理 <span class="c-gray en">&gt;</span> 部门列表 <a
	...
	<div class="pd-20">
		...
		<div class="text-c">
			...
			部门编码：
			<input type="text" name="code" id="code" placeholder="部门编码" style="width:250px" class="input-text" value="${query.code }">
			部门名称：
			<input type="text" name="name" id="name" placeholder="部门名称" style="width:250px" class="input-text" value="${query.name }">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				onclick="depart_add('添加部门','${ctx}/system/toAddDepart.do','800')"
		...
		<div class="mt-20">
			<table class="table table-border table-bordered table-hover table-bg table-sort">
				...
				<tbody>
					<c:forEach var="dataObj" items="${biVoteDeparts}">
						<tr class="text-c">
							<td><input type="checkbox" id="departId" name="departId" value="${dataObj.id }"></td>
							<td>${dataObj.brandCode }</td>
							...
							<td><a title="编辑" href="javascript:;" onclick="depart_add('编辑部门','${ctx}/system/toAddDepart.do?id=${dataObj.id}','800')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<script type="text/javascript">
		...
		function datadel(){
			var vote = "";
			var i = 0;
			$("input[name='departId']:checkbox").each(function() {
				...
				layer.confirm("是否删除选中的品牌?",function(){
					$.ajax({
						type : 'POST',
						url : ctx + '/system/deleteDepart.do',
						data : {departId:vote},
						dataType : 'json',

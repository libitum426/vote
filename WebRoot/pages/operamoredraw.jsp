...
<script type="text/javascript">
	...
	$(function() {
		...
		start_btn.click(function() {
			$.getJSON(ctx + '/getDrawData.do?drawId=' + drawId,function(data) {
				if (data) {
					...
					var drawId = data.attributes.drawconfig.id;
					...
					if(obj == null || len <= 0 || len < everyNum){
						alert("抽奖人数不足！");
						$("#closed").click();
						return false;
					}
					_gogo = setInterval(
						function() {
							...
							for (var j = 1; j <= everyNum; j++) {
								var num = getDrawRand(obj,myArray,len);
								/**
								var num = Math.floor(Math.random() * len);//获取随机数
								if (myArray.indexOf(obj[num].code) == 0) {
								...
								if (myArray.indexOf(obj[num].code) == -1) {
								...
								myArray.push(obj[num].code);
								var str = obj[num].code.substr(0, 3) + "新年快乐" + obj[num].code.substr(7, 11);
								var result = obj[num].name+"-"+str;
								...
								sts2 += "<input type='hidden' name='userId' value='"+obj[num].id+"'>"
							} 
		...
		stop_btn.click(function() {
			...
			$.post(ctx + "/updateDrawData.do", {
				...
				if (data.success) {
					var html = "";
					var obj = data.obj;//将JSON字符串转化为对象 
					var len = obj.length;
					for (var i = 0; i < len; i++) {
						var str = obj[i].code.substr(0, 3) + "新年快乐" + obj[i].code.substr(7, 11);
						var result = obj[i].departName+"-"+obj[i].name+"-"+str;
						if(len <= 5){
	...
	function getDrawRand(obj,arr,len){
		var num = Math.floor(Math.random() * len);
		if(array_contain(arr,obj[num].code)){
			return getDrawRand(obj,arr,len);
		}else{
			return num;
		}
	}

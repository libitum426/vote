...
<body>
	<nav class="breadcrumb">
		...
		投票管理 <span class="c-gray en">&gt;</span> 抽奖配置列表 <a
	...
	<div class="pd-20">
		<form action="${ctx }/drawz/drawzList.do" method="post">
		<div class="text-c">
			抽奖标题：
			<input type="text" name="drawTitle" id="drawTitle" placeholder="抽奖标题" style="width:150px" class="input-text" value="${query.drawTitle }">
			是否启用:
			<span class="select-box inline">
			<select id="isFlag" name="isFlag" class="select">
				<option value=""></option>
				<option value="0" <c:if test="${query.isFlag == '0'}">selected="selected"</c:if>>启用</option>
				<option value="1" <c:if test="${query.isFlag == '1'}">selected="selected"</c:if>>禁用</option>
			</select>
			</span>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="changeIsActive('1')"
				...
				onclick="draw_add('新增抽奖配置','${ctx}/drawz/toEditDrawz.do','800')"
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加</a></span>
		</div>
		<div class="mt-20">
			<table class="table table-border table-bordered table-hover table-bg table-sort">
				<thead>
					<tr>
						<th scope="col" colspan="9">抽奖配置列表</th>
					</tr>
					<tr class="text-c">
				...
				<tbody>
					<c:forEach var="dataObj" items="${biVoteDraws}">
						<tr class="text-c">
							<td><input type="checkbox" id="drawId" name="drawId" value="${dataObj.drawId }"></td>
							<td>${dataObj.drawTitle }</td>
							<td>
								<c:if test="${dataObj.drawType == 1}">外部</c:if>
								<c:if test="${dataObj.drawType == 0}">内部</c:if>
							</td>
							<td><fmt:formatDate value="${dataObj.startDate }" pattern="yyyy-MM-dd"/> </td>
							<td><fmt:formatDate value="${dataObj.endDate }" pattern="yyyy-MM-dd"/> </td>
							<td>
								<c:if test="${dataObj.isFlag == 1}">禁用</c:if>
								<c:if test="${dataObj.isFlag == 0}">启用</c:if>
							</td>
							<td><c:if test="${dataObj.isFlag == 1}">
								<a style="text-decoration:none" href="javaScript:void()" onclick="operaDrawconfig('${dataObj.drawId}','0');">
									<i class="Hui-iconfont">&#xe60e;启用</i>
								</a>
								</c:if>
								<c:if test="${dataObj.isFlag == 0}">
								<a href="javaScript:void()" onclick="operaDrawconfig('${dataObj.drawId}','1');">
									<i class="Hui-iconfont">&#xe605;禁用</i>
								</a>
								</c:if>
								<a style="text-decoration:none" onClick="draw_add('抽奖配置编辑','${ctx}/drawz/toEditDrawz.do?drawId=${dataObj.drawId}','800')" href="javascript:;" title="编辑">
